...
function openModal(
  ...
  cxFracionada,
  priceFracionada,
  estoque  // New parameter for stock status
) {
  ...
  const addButton = document.getElementById("addButton"); // Ensure button exists
  const stockWarning = document.getElementById("stockWarning"); // Ensure warning exists
  ...
  if (estoque === 0) {
    ...
    stockWarning.style.display = "block"; // Show warning
  } else {
    addButton.style.display = "block"; // Show Add button
    stockWarning.style.display = "none"; // Hide warning
  }
